...
def train_boundary(latent_codes,
  ...
  print("positive samples before random", positive_idx)
  ...
  train_label = np.concatenate([np.ones(train_num, dtype=np.int_),
                                np.zeros(train_num, dtype=np.int_)], axis=0)
  logger.info(f'  Training: {train_num} positive, {train_num} negative.')
  # Validation set.
  val_data = np.concatenate([positive_val, negative_val], axis=0)
  val_label = np.concatenate([np.ones(val_num, dtype=np.int_),
                              np.zeros(val_num, dtype=np.int_)], axis=0)
  ...
  remaining_label = np.ones(remaining_num, dtype=np.int_)
...
def linear_interpolate(latent_code,
  ...
  print("latent_code.shape[0]= ",latent_code.shape[0])
  print("boundary.shape[0]= ", boundary.shape[0])
  print("len(boundary.shape)= ", len(boundary.shape))
  print("boundary.shape[1] ", boundary.shape[1])
  print(" latent_code.shape[-1] ", latent_code.shape[-1])
